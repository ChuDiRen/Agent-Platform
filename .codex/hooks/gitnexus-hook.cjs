...
function extractPattern(toolName, toolInput) {
  ...
  if (toolName === 'Glob') {
    ...
    // Extract meaningful words from glob patterns (skip wildcards and short segments)
    const segments = raw.split(/[\/\\]/).filter(s => s && s !== '**' && s !== '*' && s.length >= 3);
    for (const seg of segments) {
      const match = seg.match(/([a-zA-Z][a-zA-Z0-9_-]{2,})/);
