...
from app.schemas.ui_automation import UiTestExecCreate, UiTestExecUpdate


class CRUDUiTestCasesExec(CRUDBase[UiTestCasesExec, UiTestExecCreate, UiTestExecUpdate]):
    def get_multi_by_project(
        self, db: Session, *, project_id: int | None = None, skip: int = 0, limit: int = 100
    ) -> list[UiTestCasesExec]:
        query = db.query(self.model)
        if project_id is not None:
            query = query.filter(self.model.project_id == project_id)
        return query.order_by(self.model.id.desc()).offset(skip).limit(limit).all()

    def create(self, db: Session, *, obj_in: UiTestExecCreate) -> UiTestCasesExec:
    ...
    def update(self, db: Session, *, db_obj: UiTestCasesExec, obj_in: UiTestExecUpdate) -> UiTestCasesExec:
...
ui_test_cases_exec = CRUDUiTestCasesExec(UiTestCasesExec)
