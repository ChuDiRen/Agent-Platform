...
from app.schemas.api_automation import (
    ApiAutomationCase,
    ApiAutomationCaseCreate,
    ApiAutomationCaseUpdate,
    ApiTestExecCreate,
    ApiTestExecOut,
...
def _case_to_out(item) -> ApiAutomationCase:
    return ApiAutomationCase.model_validate(
        {
            "id": item.id,
            ...
            "name": item.name,
            "request": json.loads(item.request or "{}"),
...
@router.post("/cases")
def create_api_automation_case(
    payload: ApiAutomationCaseCreate,
    db: Session = Depends(get_db),
):
...
def read_api_automation_case(case_id: int, db: Session = Depends(get_db)):
...
def update_api_automation_case(
    case_id: int,
    payload: ApiAutomationCaseUpdate,
    db: Session = Depends(get_db),
):
    item = case_crud.get(db, case_id)
    if not item:
...
def delete_api_automation_case(case_id: int, db: Session = Depends(get_db)):
    item = case_crud.get(db, case_id)
    if not item:
        return fail(message="API automation case not found", code=404)
