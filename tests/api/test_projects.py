...
class TestProjectCreate:
    ...
    def test_create_project_with_password(self, client):
        ...
        assert resp.status_code == 200
        assert resp.json()["password"] == "abc123"
...
class TestProjectUpdate:
    ...
    def test_update_project_name(self, client):
        pid = client.post(PROJECT_BASE, json=_make_project()).json()["id"]
        resp = client.put(f"{PROJECT_BASE}{pid}", json={"name": "新项目名"})
        assert resp.status_code == 200
        assert resp.json()["name"] == "新项目名"
...
class TestProjectDelete:
    ...
    def test_delete_project(self, client):
        ...
        assert resp.status_code == 200
        assert client.get(f"{PROJECT_BASE}{pid}").status_code == 404
